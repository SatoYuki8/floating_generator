...
int add_float(int exp, int frac, int bit_width, FILE *fp){
  ...
  int frac_bit;     // msb + frac + G,R,S bits (frac+6 bits)
  frac_bit = frac + 6;
  ...
  //pre-shift for adder operation
  char *zero_grs;    //G, R, S bits plus alpha bits.
  zero_grs = (char *)malloc(bit_width-(frac+3)+2);
  ZeroStr(bit_width-(frac+3), zero_grs);
...
/*	  fhead, zero_grs   */
  ...
  fprintf(fp,
	  ...
	  "alt{\n"
	  "(round): par{\n"
	  "incfrac.do(0b1, lzshift.f<:>);ここ\n"
	  "z = Cs||(((Cexp+^(lzshift.amount)+0b1)+0b1)+incfrac.p)||incfrac.out;\n"
	  "}\n"
	  "else: z = Cs||((Cexp+^(lzshift.amount)+0b1)+0b1)||lzshift.f<:>;こここ\n"
	  "}\n"
	  "}\n"
	  "}\n"
	  "\n",
